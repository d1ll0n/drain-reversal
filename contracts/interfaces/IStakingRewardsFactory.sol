...
interface IStakingRewardsFactory {
  ...
  function increaseStakingRewards(address stakingToken, uint88 rewardAmount)
    external;

  function setRewardsDuration(address stakingToken, uint256 newDuration)
    external;
  ...
  function computeStakingRewardsAddress(address stakingToken)
    external
    view
    returns (address);
}
