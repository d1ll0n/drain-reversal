...
import { getBigNumber, getContract, withSigner } from './chain'
import { SUSHISWAP_FACTORY_ADDRESS, UNISWAP_FACTORY_ADDRESS, WETH } from './constants'
...
export async function createBalanceCheckpoint(token: IERC20 | null, account: string) {
  ...
  return async (tx?: ContractTransaction | Promise<ContractTransaction>) => {
    ...
    if (!token && tx) {
      ...
      balanceAfter = (await bal()).add(gasPrice.mul(gasUsed))
    } else {
      balanceAfter = await bal()
    }
...
//#region transfers
export const sendEtherTo = (address: string, amount: BigNumber = getBigNumber(10)) =>
  withSigner(WETH, async (signer) => {
...
export function computeUniswapPairAddress(tokenA: string, tokenB: string): string {
  ...
  const salt = keccak256(Buffer.concat([toAddressBuffer(token0), toAddressBuffer(token1)]))
  return getCreate2Address(UNISWAP_FACTORY_ADDRESS, salt, initCodeHash)
}

export function computeSushiswapPairAddress(tokenA: string, tokenB: string): string {
...
export async function getTokenSymbol(token: string) {
  try {
    const metadata: IERC20Metadata = await getContract(token, 'IERC20Metadata');
    const symbol = await metadata.symbol();
    return symbol;
  } catch (err) {
    try {
      const metadata: IERC20MetadataBytes32 = await getContract(token, 'IERC20MetadataBytes32');
      const symbol = toUtf8String(await metadata.symbol());
      return symbol;
    } catch (err1) {
      console.log(`ERROR FOR https://etherscan.io/address/${token}`)
      throw err1
    }
  }
}